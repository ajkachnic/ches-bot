...
const XX = '<g id="xx"><path d="M35.865 9.135a1.89 1.89 0 0 1 0 2.673L25.173 22.5l10.692 10.692a1.89 1.89 0 0 1 0 2.673 1.89 1.89 0 0 1-2.673 0L22.5 25.173 11.808 35.865a1.89 1.89 0 0 1-2.673 0 1.89 1.89 0 0 1 0-2.673L19.827 22.5 9.135 11.808a1.89 1.89 0 0 1 0-2.673 1.89 1.89 0 0 1 2.673 0L22.5 19.827 33.192 9.135a1.89 1.89 0 0 1 2.673 0z" fill="#000" stroke="#fff" stroke-width="1.688"/></g>'

const CHECK_GRADIENT = '<radialGradient id="check_gradient"><stop offset="0%" stop-color="#ff0000" stop-opacity="1.0" /><stop offset="50%" stop-color="#e70000" stop-opacity="1.0" /><stop offset="100%" stop-color="#9e0000" stop-opacity="0.0" /></radialGradient>'
...
export class SvgRenderer {
  ...
  svg(viewbox: number, size? : number): XMLBuilder {
    ...
    const svg = fragment().ele('svg', options)
    return svg
  }
  ...
  board(fen: string, {size = 400, orientation = 'w'}: {
    ...
    for (const color of ['w', 'b']) {
      for (const p of ['p', 'n', 'b', 'r', 'q', 'n', 'k']) {
        const piece = color === 'w' ? p.toUpperCase() : p.toLowerCase()
        // @ts-expect-error
        const element = create(PIECES[piece])
    ...
    board.forEach((row, rankIndex) => {
      row.forEach((piece, fileIndex) => {
        ...
        if (piece) {
          const name = `#${shortColorToLong(piece.color)}-${shortTypeToLong(piece.type)}`
          svg.ele('use', {
            'xlink:href': name,
